...
#
#     PHX_SERVER=true bin/grocery_aid start
#
# Alternatively, you can use `mix phx.gen.release` to generate a `bin/server`
# script that automatically sets the env var above.
if System.get_env("PHX_SERVER") do
  config :grocery_aid, GroceryAidWeb.Endpoint, server: true
end
...
if config_env() == :prod do
  ...
  config :grocery_aid, GroceryAid.Repo,
    # In-cluster CNPG connection. Postgrex 0.22+ negotiates SSL by default
    # and the CNPG server CA isn't in this runtime image, so an explicit
    # `ssl: false` keeps boot migrations from failing on "TLS Unknown CA".
    # Acceptable for pod-to-pod traffic inside the cluster.
    ssl: false,
    url: database_url,
  ...
  config :grocery_aid, :dns_cluster_query, System.get_env("DNS_CLUSTER_QUERY")
  ...
  #
  #     config :grocery_aid, GroceryAid.Mailer,
