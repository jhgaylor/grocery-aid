...
#
# Phoenix release image for Grocery Aid. Multi-stage: a build stage
# compiles assets + the Elixir release against Erlang/OTP 28 + Elixir
# 1.19, and a slim Debian runtime stage carries only the release.
#
# Mirrors the home-cloud convention used by `guild` (single-replica
...
ENV LANG=C.UTF-8 \
    ...
    PHX_SERVER=true \
    PORT=4000 \
    PHX_HOST=grocery-aid.inevitable.fyi
...
COPY --from=build /app/_build/prod/rel/grocery_aid ./
...
CMD ["/bin/sh", "-c", "/app/bin/grocery_aid eval 'GroceryAid.Release.migrate()' && /app/bin/grocery_aid eval 'GroceryAid.Release.seed()' && exec /app/bin/grocery_aid start"]
